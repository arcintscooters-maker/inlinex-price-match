...
function matchAll(shopifyProducts, iwProducts, xtProducts, isProducts) {
  ...
  //   2. Otherwise fall back to Jaccard similarity on the target name.
  {
    ...
    for (const [deadSku, targets] of [...xtManualMap.entries()]) {
      ...
      // similarity. Strip common skating stopwords first so "Aggressive
      // Skate" vs "Inline Skates" doesn't tank the score — these words
      // appear on almost every product and can't disambiguate.
    ...
    if (needsPersist) {
      ...
      try {
        ...
        // Remove any leftover entries still pointing to dead SKUs that
        // were successfully rebound. This cleans up URL-based duplicates
        // the colleague added for the same product under the old SKU.
        if (reboundDeadSkus.size > 0) {
          ...
          data.mappings = data.mappings.filter(m => {
            ...
            return !reboundDeadSkus.has(m.sku || m.iwSku);
          });
