...
// Leaf-level category codes per brand (these have actual product listings)
const BRAND_CATEGORIES = {
  'Powerslide': ['PSRECS', 'PSCTS', 'PSURB', 'PSSPD', 'PSKIDS', 'PSWH', 'PSFRAMES', 'PSPROTECT'],
  'USD': ['USDAGG', 'USDWH', 'USDFRAMES'],
  'Rollerblade': ['RBRECS', 'RBCTS', 'RBKIDS', 'RBWH', 'RBFRAMES'],
  'FR Skates': ['FRRECS', 'FRCTS', 'FRURB', 'FRWH', 'FRFRAMES'],
  'Flying Eagle': ['FERECS', 'FECTS', 'FEKIDS'],
  'K2': ['K2RECS', 'K2CTS', 'K2KIDS', 'K2WH'],
  'Seba': ['SEBARECS', 'SEBACTS', 'SEBAURB', 'SEBASPD'],
  'Ennui': ['ENNUIHELM', 'ENNUIKNEE', 'ENNUIWRIST', 'ENNUIELBOW'],
};

// Broader category codes as fallback (these pages list products from all brands)
const GENERAL_CATEGORIES = [
  'FITSK8', 'RST', 'CTST', 'UST', 'AGT', 'SKT',
  'FITJRSKATE', 'FSST',
...
function extractProductFromPage(html, url) {
  ...
  // Strategy: look for the js-ordering-price which is the actual price used for orders
  let price = null;

  // Try js-ordering-price first (most reliable)
  const orderPriceMatch = html.match(/class="js-ordering-price[^"]*"[^>]*>([\d.]+)/);
  if (orderPriceMatch) {
    price = parseFloat(orderPriceMatch[1]);
  }

  // Try afterpay-full_price (another reliable source)
  if (!price) {
    const afterpayMatch = html.match(/class="afterpay-full_price"[^>]*>([\d.]+)/);
    if (afterpayMatch) {
      price = parseFloat(afterpayMatch[1]);
    }
  }
  ...
  if (!price) {
    ...
    if (salePriceMatch) {
      price = parseFloat(salePriceMatch[1]);
    }
  }

  // Fallback: find lowest dollar amount in the price area
  if (!price) {
    const priceAreaMatch = html.match(/desc_top-head-price[\s\S]{0,500}/);
    if (priceAreaMatch) {
      const dollarAmounts = priceAreaMatch[0].match(/\$([\d]+(?:\.\d{2})?)/g);
      if (dollarAmounts && dollarAmounts.length > 0) {
        const amounts = dollarAmounts.map(d => parseFloat(d.replace('$', ''))).filter(a => a > 10);
        if (amounts.length > 0) price = Math.min(...amounts);
      }
    }
  }
