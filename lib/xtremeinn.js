...
async function getProductUrlsWithPuppeteer(brands) {
  ...
  try {
    ...
    // Load all products by clicking "View more products" button repeatedly
    // Button is .btn-cargar-mas — need to scroll down first to make it visible
    for (let click = 0; click < 20; click++) {
      ...
      // Try clicking the button — use multiple selectors
      const clicked = await page.evaluate(() => {
        // Try the main load more button
        const btn = document.querySelector('.btn-cargar-mas');
        if (btn) { btn.scrollIntoView(); btn.click(); return 'btn-cargar-mas'; }
        // Try Algolia infinite hits button
        const algBtn = document.querySelector('.ais-InfiniteHits-loadMore');
        if (algBtn) { algBtn.scrollIntoView(); algBtn.click(); return 'ais-loadMore'; }
        // Try any "View more" link/button
        const allBtns = Array.from(document.querySelectorAll('button, a'));
        const viewMore = allBtns.find(b => b.textContent.toLowerCase().includes('view more product'));
        if (viewMore) { viewMore.scrollIntoView(); viewMore.click(); return 'viewMore'; }
        return null;
      });
