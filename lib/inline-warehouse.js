...
function extractProductsFromPage(html, url) {
  ...
  for (const tr of trs) {
    ...
    // A row can have multiple style-item-name spans: typically one is the
    // raw part number (e.g. "940472") and the next is the readable label
    // ("Next Cuff SM (36-39)"). Grab them all and pick the most descriptive.
    const allNames = [...tr.matchAll(/js-ordering-style-item-name[^>]*>([^<]+)/g)]
      .map(m => m[1].trim())
      .filter(Boolean);
    const readable = allNames.find(n => !/^[\d-]+$/.test(n)) || allNames[0];
    const styleItemName = readable;
    ...
    const nameSlug = styleItemName.replace(/[^a-z0-9]/gi, '').toLowerCase();
    // Start with the simple old-format SKU; we disambiguate after extraction
    // if two rows end up with the same SKU but different readable names.
    const baseSkuSuffix = styleItemCode || nameSlug;

    variants.push({
      name: `${baseName} ${styleItemName}`,
      sku: `${baseSku}-${baseSkuSuffix}`,
      _nameSlug: nameSlug,
      price,
  ...
  // Dedupe: collapse rows that share both SKU and readable name (e.g. same
  // color repeated across 6 size rows). Rows with the same SKU but different
  // readable names (e.g. Powerslide Next NRCB appearing twice, once as "SM"
  // and once as "LG") must survive dedup.
  const seenKey = new Set();
  const deduped = [];
  for (const v of variants) {
    const key = `${v.sku}||${v._nameSlug}`;
    if (seenKey.has(key)) continue;
    seenKey.add(key);
    deduped.push(v);
  }

  // If after dedup the same SKU still appears more than once, those are
  // genuinely different variants sharing a styleitemcode — disambiguate by
  // appending the name slug to each.
  const skuCounts = {};
  for (const v of deduped) skuCounts[v.sku] = (skuCounts[v.sku] || 0) + 1;
  for (const v of deduped) {
    if (skuCounts[v.sku] > 1 && v._nameSlug) {
      v.sku = `${v.sku}-${v._nameSlug}`;
    }
    delete v._nameSlug;
  }
  ...
  // If we extracted exactly 1 variant from the ordering table, use it
  // directly instead of re-extracting from the page (which could get a
  // different price from a different HTML element).
  if (result.length === 1) {
    return result;
  }

  // --- Single variant product (no ordering table at all) ---
  let price = null;
