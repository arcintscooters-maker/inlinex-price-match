...
function matchAll(shopifyProducts, iwProducts, xtProducts, isProducts) {
  ...
  // rebind the mapping to the new SKU in-memory.
  {
    ...
    for (const [deadSku, targets] of [...xtManualMap.entries()]) {
      ...
      // Score every live xt product against the target by Jaccard token
      // similarity (same function the fuzzy matcher uses). Rebind to the
      // top scorer only if:
      //   - it clears a minimum similarity threshold (0.70)
      //   - it beats the runner-up by a meaningful margin (>= 0.12)
      // This is more forgiving than strict all-tokens matching (so rotations
      // actually get picked up) but still blocks ambiguous picks.
      for (const target of targets) {
        ...
        if (best && bestScore >= 0.70 && (bestScore - second) >= 0.12) {
          if (!xtManualMap.has(best.sku)) xtManualMap.set(best.sku, []);
          xtManualMap.get(best.sku).push(target);
          rebindings.push({ dead: deadSku, live: best.sku, target, name: best.name, score: bestScore });
        } else if (best && bestScore >= 0.70) {
          log('MATCH', `  Rebind skipped for xt:${deadSku} "${target}" — top ${bestScore.toFixed(2)} too close to runner-up ${second.toFixed(2)}`);
        }
      }
    }
    if (rebindings.length > 0) {
      ...
      for (const r of rebindings.slice(0, 20)) {
        log('MATCH', `  xt:${r.dead} -> xt:${r.live} | "${r.target.slice(0,60)}" matched "${r.name}" (sim ${r.score.toFixed(2)})`);
      }
