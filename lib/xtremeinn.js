...
async function backfillMappedProducts(scrapedProducts, markets, shopifyProductsInScope) {
  ...
  if (!data || !data.mappings) return { usProducts: [], auProducts: [], idProducts: [], phProducts: [], jpProducts: [], caProducts: [] };
  ...
  if (missing.length === 0) return { usProducts: [], auProducts: [], idProducts: [], phProducts: [], jpProducts: [], caProducts: [] };
  ...
  log('XT', `Backfill: +${usProducts.length} US, +${auProducts.length} AU, +${idProducts.length} ID, +${phProducts.length} PH, +${jpProducts.length} JP, +${caProducts.length} CA`);
  return { usProducts, auProducts, idProducts, phProducts, jpProducts, caProducts };
}
...
async function scrapeAll(brands, markets, shopifyProductsInScope) {
  ...
  if (enableUS && !enableAU && !enableID && !enablePH && !enableJP && !enableCA) {
  ...
  // SLOW PATH: AU/ID/PH/JP/CA or multiple markets — need individual product pages for EUR
  ...
  const scrapeOne = async (url) => {
    try {
      ...
      scraped++;
      if (scraped % 50 === 0) {
        log('XT', `Progress: ${scraped}/${urls.length} scraped, US:${usProducts.length} AU:${auProducts.length} ID:${idProducts.length} PH:${phProducts.length} JP:${jpProducts.length} CA:${caProducts.length}`);
      }
    } catch (e) {
      failed++;
  ...
  const allScraped = [...usProducts, ...auProducts, ...idProducts, ...phProducts, ...jpProducts, ...caProducts];
  ...
  log('XT', `=== xtremeinn complete: US:${usProducts.length} AU:${auProducts.length} ID:${idProducts.length} PH:${phProducts.length} JP:${jpProducts.length} CA:${caProducts.length} (${unavailable} unavail, ${failed} errors) ===`);
  return { usProducts, auProducts, idProducts, phProducts, jpProducts, caProducts };
}
