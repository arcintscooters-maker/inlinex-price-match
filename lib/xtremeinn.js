...
function extractProductFromPage(html, url, forMarket) {
  ...
  const slugMatch = url.match(/\/xtremeinn\/en\/([^/]+)\/\d+\/p/);
...
async function backfillMappedProducts(scrapedProducts, markets) {
  ...
  if (!data || !data.mappings) return { usProducts: [], auProducts: [], idProducts: [] };
  ...
  if (missing.length === 0) return { usProducts: [], auProducts: [], idProducts: [] };
  ...
  const fetchOne = async (sku) => {
    ...
    try {
      ...
      if (enableUS) {
        const p = extractProductFromPage(body, url, 'US');
        if (p) usProducts.push(p);
      }
      if (enableAU) {
  ...
  log('XT', `Backfill: +${usProducts.length} US, +${auProducts.length} AU, +${idProducts.length} ID`);
...
async function scrapeAll(brands, markets) {
  ...
  const allScraped = [...usProducts, ...auProducts, ...idProducts];
