...
const EUR_TO_PHP = 76.3;  // calibrated from tradeinn PH product page
const EUR_TO_JPY = 201.7; // calibrated from tradeinn JP product page (Powerslide Tau 90)
const EUR_TO_CAD = 1.79;  // calibrated from tradeinn CA product page (USD Aeon 60 Mery Munoz)
const EUR_TO_NZD = 2.162; // calibrated from tradeinn NZ product page (USD Aeon 60 Mery Munoz Pro)
...
function extractProductFromPage(html, url, forMarket) {
  ...
  if (forMarket === 'US' && localPrice) {
    ...
    currency = 'PHP';
  } else if (forMarket === 'JP') {
    price = Math.round(eurPrice * EUR_TO_JPY);
    currency = 'JPY';
  } else if (forMarket === 'CA') {
    price = Math.round(eurPrice * EUR_TO_CAD * 100) / 100;
    currency = 'CAD';
  } else if (forMarket === 'NZ') {
    price = Math.round(eurPrice * EUR_TO_NZD * 100) / 100;
    currency = 'NZD';
  } else {
      ...
      try {
        ...
        for (let click = 0; click < 30; click++) {
          ...
          const prevCount = await page.evaluate(() =>
            document.querySelectorAll('li.product-listing-wrapper.js-hover-producto').length);
          ...
          if (!hasMore) break;
          // Wait for new products to appear (max 2.5s) instead of fixed sleep
          await page.waitForFunction(
            (prev) => document.querySelectorAll('li.product-listing-wrapper.js-hover-producto').length > prev,
            { timeout: 2500 },
            prevCount
          ).catch(() => {});
        }
 ...
 * still fire.
 */
async function backfillMappedProducts(scrapedProducts, markets, shopifyProductsInScope) {
  const { loadJSON } = require('./utils');
  const path = require('path');
  const mappingFile = path.join(__dirname, '..', 'manual-mappings.json');
  const data = loadJSON(mappingFile);
  if (!data || !data.mappings) return { usProducts: [], auProducts: [], idProducts: [], phProducts: [], jpProducts: [], caProducts: [], nzProducts: [] };

  // Build a haystack of in-scope Shopify products so we only backfill mappings
  // that could actually match something in the current brand filter. This
  // prevents pulling in recycled xt SKUs for brands the user didn't select.
  let inScopeHaystack = null;
  if (Array.isArray(shopifyProductsInScope) && shopifyProductsInScope.length > 0) {
    inScopeHaystack = new Set();
    for (const p of shopifyProductsInScope) {
      if (p.title) inScopeHaystack.add(p.title.toLowerCase());
      if (p.handle) inScopeHaystack.add(p.handle.toLowerCase());
      for (const v of (p.variants || [])) {
        if (v.sku) inScopeHaystack.add(String(v.sku).toLowerCase());
        if (v.barcode) inScopeHaystack.add(String(v.barcode).toLowerCase());
      }
    }
  }

  const mappingTargetInScope = (mapping) => {
    if (!inScopeHaystack) return true; // no filter provided — allow all
    const target = (mapping.shopifyMatch || '').toLowerCase().trim();
    if (!target) return false;
    if (target === '__blocked__') return false;
    // URL? Extract handle
    const handleMatch = target.match(/\/products\/([^?#]+)/);
    const candidates = [target];
    if (handleMatch) candidates.push(handleMatch[1]);
    // Reject suspiciously-short candidates — garbage URLs like "products/11"
    // would match almost any variant SKU/barcode in the haystack.
    const MIN_CAND_LEN = 5;
    const validCandidates = candidates.filter(c => c.length >= MIN_CAND_LEN);
    if (validCandidates.length === 0) return false;
    // Check if target or any substring matches an in-scope product
    for (const cand of validCandidates) {
      if (inScopeHaystack.has(cand)) return true;
      // Substring match — require the shorter side also be >= MIN_CAND_LEN
      // to avoid 4-char haystack items (SKU fragments) matching too greedily.
      for (const item of inScopeHaystack) {
        if (item.length < MIN_CAND_LEN) continue;
        if (item.includes(cand) || cand.includes(item)) return true;
      }
    }
    return false;
  };

  const inScopeMappings = data.mappings.filter(m =>
    (m.source || 'iw') === 'xt' &&
    m.shopifyMatch !== '__blocked__' &&
    mappingTargetInScope(m)
  );
  const mappedXtSkus = new Set(inScopeMappings.map(m => m.sku));
  const scrapedSkus = new Set(scrapedProducts.map(p => p.sku));
  const missing = [...mappedXtSkus].filter(sku => !scrapedSkus.has(sku));
  if (inScopeHaystack) {
    const totalXtMappings = data.mappings.filter(m => (m.source || 'iw') === 'xt').length;
    log('XT', `Backfill scope: ${inScopeMappings.length}/${totalXtMappings} xt mappings match current brand filter`);
  }
  if (missing.length === 0) return { usProducts: [], auProducts: [], idProducts: [], phProducts: [], jpProducts: [], caProducts: [], nzProducts: [] };
  ...
  const enableJP = markets && markets.includes('JP');
  const enableCA = markets && markets.includes('CA');
  const enableNZ = markets && markets.includes('NZ');
  ...
  // Build an allowed-brand set from in-scope Shopify products so we can reject
  // backfilled products whose brand doesn't match (e.g. SKU recycled from a
  // USD skate into a Powerslide one would pass the name-similarity check but
  // still be out of scope).
  const allowedBrands = inScopeHaystack ? new Set() : null;
  if (allowedBrands && Array.isArray(shopifyProductsInScope)) {
    for (const p of shopifyProductsInScope) {
      if (p.vendor) allowedBrands.add(p.vendor.toLowerCase());
    }
  }
  ...
  const fetchOne = async (sku) => {
    ...
    try {
      ...
      if (status !== 200) {
        notFound++;
        if (notFound <= 10) log('XT', `  DEAD SKU ${sku}: HTTP ${status} — product removed from xtremeinn`);
        return;
      }
      const testProduct = extractProductFromPage(body, url, 'US');
      if (!testProduct) {
        notFound++;
        if (notFound <= 10) log('XT', `  DEAD SKU ${sku}: page redirected/invalid — product removed from xtremeinn`);
        return;
      }
      ...
      // Brand-scope check: reject when the user picked a brand and the
      // backfilled product's brand doesn't overlap with any allowed brand.
      if (allowedBrands && allowedBrands.size > 0) {
        const prodBrand = (testProduct.brand || '').toLowerCase();
        const brandOk = !prodBrand || [...allowedBrands].some(b => b.includes(prodBrand) || prodBrand.includes(b));
        if (!brandOk) {
          wrongBrand++;
          if (wrongBrand <= 10) {
            log('XT', `  WRONG-BRAND SKU ${sku}: "${testProduct.name}" (brand "${testProduct.brand}") not in selected scope — skipping`);
          }
          return;
        }
      }
      ...
      if (enableJP) {
        const p = extractProductFromPage(body, url, 'JP');
        if (p) jpProducts.push(p);
      }
      if (enableCA) {
        const p = extractProductFromPage(body, url, 'CA');
        if (p) caProducts.push(p);
      }
      if (enableNZ) {
        const p = extractProductFromPage(body, url, 'NZ');
        if (p) nzProducts.push(p);
      }
    } catch (e) {
  ...
  if (wrongBrand > 0) log('XT', `Backfill: rejected ${wrongBrand} SKUs with brand outside current scope`);
  if (notFound > 0) log('XT', `Backfill: ${notFound} dead SKUs (product removed from xtremeinn catalog) — their mappings need to be deleted or re-pointed`);
  log('XT', `Backfill: +${usProducts.length} US, +${auProducts.length} AU, +${idProducts.length} ID, +${phProducts.length} PH, +${jpProducts.length} JP, +${caProducts.length} CA, +${nzProducts.length} NZ`);
  return { usProducts, auProducts, idProducts, phProducts, jpProducts, caProducts, nzProducts };
}

async function scrapeAll(brands, markets, shopifyProductsInScope) {
  ...
  if (enableUS && !enableAU && !enableID && !enablePH && !enableJP && !enableCA && !enableNZ) {
    ...
    const backfill = await backfillMappedProducts(usProducts, markets, shopifyProductsInScope);
    usProducts.push(...backfill.usProducts);
    log('XT', `=== xtremeinn fast scrape complete: US:${usProducts.length} ===`);
    return { usProducts, auProducts: [], idProducts: [], phProducts: [], jpProducts: [], caProducts: [], nzProducts: [] };
  }

  // SLOW PATH: AU/ID/PH/JP/CA/NZ or multiple markets — need individual product pages for EUR
  ...
  const scrapeOne = async (url) => {
    try {
      ...
      if (status === 200) {
        ...
        if (enableJP) {
          const jpProd = extractProductFromPage(body, url, 'JP');
          if (jpProd) jpProducts.push(jpProd);
        }
        if (enableCA) {
          const caProd = extractProductFromPage(body, url, 'CA');
          if (caProd) caProducts.push(caProd);
        }
        if (enableNZ) {
          const nzProd = extractProductFromPage(body, url, 'NZ');
          if (nzProd) nzProducts.push(nzProd);
        }
      }
      scraped++;
      if (scraped % 50 === 0) {
        log('XT', `Progress: ${scraped}/${urls.length} scraped, US:${usProducts.length} AU:${auProducts.length} ID:${idProducts.length} PH:${phProducts.length} JP:${jpProducts.length} CA:${caProducts.length} NZ:${nzProducts.length}`);
      }
    } catch (e) {
      failed++;
  ...
  const allScraped = [...usProducts, ...auProducts, ...idProducts, ...phProducts, ...jpProducts, ...caProducts, ...nzProducts];
  const backfill = await backfillMappedProducts(allScraped, markets, shopifyProductsInScope);
  usProducts.push(...backfill.usProducts);
  auProducts.push(...backfill.auProducts);
  idProducts.push(...backfill.idProducts);
  phProducts.push(...(backfill.phProducts || []));
  jpProducts.push(...(backfill.jpProducts || []));
  caProducts.push(...(backfill.caProducts || []));
  nzProducts.push(...(backfill.nzProducts || []));

  log('XT', `=== xtremeinn complete: US:${usProducts.length} AU:${auProducts.length} ID:${idProducts.length} PH:${phProducts.length} JP:${jpProducts.length} CA:${caProducts.length} NZ:${nzProducts.length} (${unavailable} unavail, ${failed} errors) ===`);
  return { usProducts, auProducts, idProducts, phProducts, jpProducts, caProducts, nzProducts };
}
