    ...
    for (const { brand, path } of pagesToScrape) {
      ...
      try {
        ...
        const cards = await page.evaluate(() => {
          const list = [];
          const cardEls = document.querySelectorAll('li.product-listing-wrapper_carrousel');
...
async function scrapeAll(brands, markets) {
  ...
  if (enableUS && !enableAU) {
    ...
    // Note: if running from non-US IP, prices may be in other currencies
    // Railway runs in US, so they'll be USD
    log('XT', `=== xtremeinn fast scrape complete: US:${usProducts.length} ===`);
    return { usProducts, auProducts: [] };
  }

  // SLOW PATH: AU or both markets — need individual product pages for EUR
