  ...
  for (; pageNum <= MAX_PAGES; pageNum++) {
    ...
    try {
      ...
      if (pageNum === 1) {
        ...
        if (total !== null) {
          log('IS', `${category}: ${total} total products`);
        }
      }
      ...
      // Don't filter by brand here — IndoSkates uses generic names like "Crossfire"
      // without the brand prefix. The matcher will handle brand matching downstream.
      products.push(...pageProducts);

      // Stop if we've collected all products
