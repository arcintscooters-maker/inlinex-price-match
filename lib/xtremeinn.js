...
async function getProductUrlsWithPuppeteer(brands) {
  ...
  try {
    ...
    // Scroll down repeatedly to load all products (infinite scroll)
    // xtremeinn loads ~96 products per scroll batch
    let previousCount = 0;
    let stableCount = 0;

    for (let scroll = 0; scroll < 60; scroll++) {
      // Extract product links currently in the DOM
      ...
      if (allUrls.size === previousCount) {
        stableCount++;
        if (stableCount >= 5) {
          log('XT', `No new products after ${scroll + 1} scrolls. Total: ${allUrls.size}`);
          break;
        }
      } else {
        stableCount = 0;
        if (scroll % 5 === 0 || allUrls.size > previousCount + 20) {
          log('XT', `Scroll ${scroll + 1}: ${allUrls.size} products found`);
        }
      }
      previousCount = allUrls.size;

      // Scroll incrementally (some sites need gradual scrolling, not jump to bottom)
      await page.evaluate(() => window.scrollBy(0, 1500));
      await sleep(1500);

      // Every 10 scrolls, also try scrolling to absolute bottom to trigger lazy loaders
      if (scroll % 10 === 9) {
