...
function matchAll(shopifyProducts, iwProducts, xtProducts, isProducts) {
  ...
  //   2. Otherwise fall back to Jaccard similarity on the target name.
  {
    ...
    // "Inline Skates" doesn't tank the score. IMPORTANT: do NOT strip
    // gender/age words (women/men/kids/junior) — those are distinguishing
    // features for catalog rotations (e.g. RB 80 vs RB 80 W Woman). Instead
    // stem plurals so "women"/"woman", "skates"/"skate", "frames"/"frame"
    // are treated as the same token.
    const REBIND_STOP = new Set([
      'skate', 'inline', 'aggressive', 'adjustable',
      'usd', 'powerslide', 'rollerblade', 'fr', 'seba', 'chaya',
    ]);
    const SYNONYMS = new Map([
      ['women', 'woman'], ['mens', 'man'], ['men', 'man'],
      ['kid', 'kids'], ['junior', 'junior'], ['juniors', 'junior'],
    ]);
    const stemWord = (w) => {
    ...
    const stripStop = (str) =>
      normalize(str).split(' ')
        .map(stemWord)
        .filter(w => w.length >= 2 && !REBIND_STOP.has(w))
        .join(' ');
    // Pure Jaccard for rebind — no substring bonus so that "rb 80 woman" vs
    // "rb 80" scores 0.67, not 0.97 (which tanked the gap to runner-up).
    const rebindSim = (a, b) => {
      if (!a || !b) return 0;
      if (a === b) return 1;
    ...
    for (const [deadSku, targets] of [...xtManualMap.entries()]) {
      ...
      for (const target of targets) {
        ...
        // Compare with stopwords stripped + pure Jaccard
        const targetClean = stripStop(comparisonTarget);
        let best = null, bestScore = 0;
        let second = 0;
        for (const p of xtProducts) {
          if (!p.name) continue;
          const score = rebindSim(targetClean, stripStop(p.name));
          if (score > bestScore) {
            second = bestScore;
            bestScore = score;
            best = p;
