...
async function getMarketPriceLists() {
  ...
  if (!usPriceList || !auPriceList || !idPriceList || !phPriceList || !jpPriceList || !caPriceList) {
    log('SHOPIFY', `All price lists found: ${lists.map(pl => `${pl.name} (${pl.currency})`).join(', ')}`);
  }

  return { usPriceList, auPriceList, idPriceList, phPriceList, jpPriceList, caPriceList };
}
