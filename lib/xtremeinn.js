...
const EUR_TO_JPY = 190;   // placeholder — calibrate from a tradeinn JP product page after first run
const EUR_TO_CAD = 1.65;  // placeholder — calibrate from a tradeinn CA product page after first run
...
async function backfillMappedProducts(scrapedProducts, markets, shopifyProductsInScope) {
  ...
  const mappingTargetInScope = (mapping) => {
    ...
    for (const cand of candidates) {
      ...
      // Substring match against any in-scope title (e.g. mapping target "Shadow Team 60" matches title "USD Shadow Team 60 Aggressive Skates")
