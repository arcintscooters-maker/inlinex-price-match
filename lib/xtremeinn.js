...
async function backfillMappedProducts(scrapedProducts, markets, shopifyProductsInScope) {
  ...
  if (!data || !data.mappings) return { usProducts: [], auProducts: [], idProducts: [], phProducts: [], jpProducts: [] };
  ...
  if (missing.length === 0) return { usProducts: [], auProducts: [], idProducts: [], phProducts: [], jpProducts: [] };
  ...
  log('XT', `Backfill: +${usProducts.length} US, +${auProducts.length} AU, +${idProducts.length} ID, +${phProducts.length} PH, +${jpProducts.length} JP`);
  return { usProducts, auProducts, idProducts, phProducts, jpProducts };
}
...
async function scrapeAll(brands, markets, shopifyProductsInScope) {
  ...
  if (enableUS && !enableAU && !enableID && !enablePH && !enableJP) {
  ...
  // SLOW PATH: AU/ID/PH/JP or multiple markets — need individual product pages for EUR
  ...
  const scrapeOne = async (url) => {
    try {
      ...
      scraped++;
      if (scraped % 50 === 0) {
        log('XT', `Progress: ${scraped}/${urls.length} scraped, US:${usProducts.length} AU:${auProducts.length} ID:${idProducts.length} PH:${phProducts.length} JP:${jpProducts.length}`);
      }
    } catch (e) {
      failed++;
  ...
  const allScraped = [...usProducts, ...auProducts, ...idProducts, ...phProducts, ...jpProducts];
  ...
  log('XT', `=== xtremeinn complete: US:${usProducts.length} AU:${auProducts.length} ID:${idProducts.length} PH:${phProducts.length} JP:${jpProducts.length} (${unavailable} unavail, ${failed} errors) ===`);
  return { usProducts, auProducts, idProducts, phProducts, jpProducts };
}
