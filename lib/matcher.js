...
function matchAll(shopifyProducts, iwProducts, xtProducts, isProducts) {
  ...
  // rebind the mapping to the new SKU in-memory.
  {
    ...
    for (const [deadSku, targets] of [...xtManualMap.entries()]) {
      ...
      // Pick a live xt product whose name shares the most tokens with the target.
      // Target is already lowercased.
      for (const target of targets) {
        if (target === '__blocked__') continue;
        const targetTokens = new Set(normalize(target).split(' ').filter(w => w.length >= 3));
        ...
        for (const p of xtProducts) {
          ...
          const nameTokens = new Set(nameNorm.split(' ').filter(w => w.length >= 3));
          let hits = 0;
          for (const t of targetTokens) if (nameTokens.has(t)) hits++;
          // Require at least half the target tokens to match, and pick the best
          const ratio = hits / targetTokens.size;
        ...
        if (bestP) {
          // Attach this target to the live sku as a new mapping entry (don't
          // overwrite — the live sku may already have its own mappings)
