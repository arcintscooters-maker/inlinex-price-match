...
function matchAll(shopifyProducts, iwProducts, xtProducts, isProducts) {
  ...
  // xtremeinn rotates product IDs every few months but keeps the same product
  // names. When a mapping's xt SKU is no longer in the current scrape, look
  // for a live xt product whose name matches the mapping's Shopify target and
  // rebind the mapping to the new SKU in-memory.
  {
    ...
    for (const [deadSku, targets] of [...xtManualMap.entries()]) {
      ...
      // Pick a live xt product whose name contains EVERY non-trivial token of
      // the target (+ the target's numeric/colour distinguishers). If zero or
      // multiple candidates qualify, give up — a wrong guess is worse than no
      // guess, because the user then has to delete it manually later.
      // Target is already lowercased.
      for (const target of targets) {
        if (target === '__blocked__') continue;
        // Drop "usd", "aggressive", "skates", "skate" — these match everything
        // in the USD brand catalog and are too weak to disambiguate.
        const STOP = new Set(['usd', 'aggressive', 'skates', 'skate', 'inline', 'the', 'and', 'for']);
        const targetTokens = [...new Set(
          normalize(target).split(' ').filter(w => w.length >= 2 && !STOP.has(w))
        )];
        ...
        for (const p of xtProducts) {
          ...
          if (targetTokens.every(t => nameTokens.has(t))) {
            candidates.push(p);
          }
        }

        // Only rebind when exactly one live product contains all distinguishing
        // tokens. Zero = the product is gone. >1 = ambiguous, user must pick.
        if (candidates.length === 1) {
          const bestP = candidates[0];
          if (!xtManualMap.has(bestP.sku)) xtManualMap.set(bestP.sku, []);
          xtManualMap.get(bestP.sku).push(target);
          rebindings.push({ dead: deadSku, live: bestP.sku, target, name: bestP.name });
        } else if (candidates.length > 1) {
          log('MATCH', `  Rebind skipped for xt:${deadSku} "${target}" — ${candidates.length} live candidates, ambiguous`);
        }
      }
    }
    if (rebindings.length > 0) {
      ...
      for (const r of rebindings.slice(0, 20)) {
        log('MATCH', `  xt:${r.dead} -> xt:${r.live} | "${r.target}" matched "${r.name}"`);
      }
      ...
      try {
        ...
        for (const r of rebindings) {
          for (const m of data.mappings) {
            ...
            m.sku = r.live;
            m.note = `Auto-rebound from ${r.dead} (xtremeinn catalog rotation)`;
            modified++;
            break;
          }
        }
        if (modified > 0) {
          ...
          log('MATCH', `Persisted ${modified} rebind(s) to manual-mappings.json`);
        }
      } catch (e) {
  ...
  for (const product of shopifyProducts) {
    ...
    for (const p of iwProducts) {
      ...
      for (const t of targets) {
        ...
        if (titleNorm.includes(normalize(t))) {
          matched = true;
          break;
        }
      }
