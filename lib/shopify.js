...
// Auth: mint a fresh Admin API token via client_credentials (auto-rotating,
// ~24h) instead of a long-lived SHOPIFY_ACCESS_TOKEN. Falls back to a static
// token if client creds aren't configured.
...
function _mintToken() {
  return new Promise((resolve, reject) => {
    const body = JSON.stringify({ grant_type: 'client_credentials', client_id: CLIENT_ID, client_secret: CLIENT_SECRET });
    const req = https.request({
      ...
      res.on('end', () => {
        try { const j = JSON.parse(d); j.access_token ? resolve(j) : reject(new Error('token mint failed: ' + d.slice(0, 120))); }
        catch (e) { reject(new Error('token mint parse error')); }
      });
    });
    req.on('error', reject);
    req.setTimeout(15000, () => { req.destroy(); reject(new Error('token mint timeout')); });
...
async function getToken() {
  if (CLIENT_ID && CLIENT_SECRET) {
    ...
    const j = await _mintToken();
    _tok = j.access_token;
    _tokExp = Date.now() + ((j.expires_in || 86400) * 1000) - 5 * 60 * 1000; // refresh 5 min early
    return _tok;
  }
  return STATIC_TOKEN;
}
...
async function getMarketPriceLists() {
  ...
  let jpPriceList = null;
  let caPriceList = null;
  let nzPriceList = null;
  ...
  for (const pl of lists) {
    ...
    if (pl.currency === 'JPY' && !jpPriceList) {
      jpPriceList = { id: pl.id, name: pl.name, currency: pl.currency };
      log('SHOPIFY', `JP price list: ${pl.name} (${pl.id}) - ${pl.currency}`);
    }
    if (pl.currency === 'CAD' && !caPriceList) {
      caPriceList = { id: pl.id, name: pl.name, currency: pl.currency };
      log('SHOPIFY', `CA price list: ${pl.name} (${pl.id}) - ${pl.currency}`);
    }
    if (pl.currency === 'NZD' && !nzPriceList) {
      nzPriceList = { id: pl.id, name: pl.name, currency: pl.currency };
      log('SHOPIFY', `NZ price list: ${pl.name} (${pl.id}) - ${pl.currency}`);
    }
  }

  if (!usPriceList || !auPriceList || !idPriceList || !phPriceList || !jpPriceList || !caPriceList || !nzPriceList) {
    log('SHOPIFY', `All price lists found: ${lists.map(pl => `${pl.name} (${pl.currency})`).join(', ')}`);
  }

  return { usPriceList, auPriceList, idPriceList, phPriceList, jpPriceList, caPriceList, nzPriceList };
}
