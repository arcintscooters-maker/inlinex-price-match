...
function matchAll(shopifyProducts, iwProducts, xtProducts, isProducts) {
  ...
  //   2. Otherwise fall back to Jaccard similarity on the target name.
  {
    ...
    for (const [deadSku, targets] of [...xtManualMap.entries()]) {
      ...
      // top scorer only if:
      //   - it clears a minimum similarity threshold (0.65)
      //   - it beats the runner-up by a meaningful margin (>= 0.10)
      //
      // When the target is a Shopify URL like
      //   https://inlinex.com.sg/products/powerslide-swell-jr-100-kids-skates-1
      // raw similarity against an xt name would be ~0 because of the URL junk.
      // So we extract handle words ("powerslide swell jr 100 kids skates 1")
      ...
      for (const target of targets) {
        ...
        if (handleMatch) {
          // URL target — convert handle slug to words
        ...
        for (const p of xtProducts) {
          if (!p.name) continue;
          const score = similarity(comparisonTarget, p.name);
          if (score > bestScore) {
            second = bestScore;
            bestScore = score;
            best = p;
        ...
        } else if (best && bestScore >= 0.50) {
          log('MATCH', `  Rebind skipped for xt:${deadSku} "${comparisonTarget.slice(0,50)}" — top ${bestScore.toFixed(2)} runner-up ${second.toFixed(2)}`);
        }
      }
    }
      ...
      try {
        ...
        for (const r of rebindings) {
          for (const m of data.mappings) {
            ...
            m.sku = r.live;
            // Always update the stored slug so the next rotation can rebind
            // via slug lookup (instant, unambiguous) instead of similarity.
