...
async function getMarketPriceLists() {
  ...
  for (const pl of lists) {
    ...
    // Match by currency and/or name containing country hints
    if (pl.currency === 'USD' && !usPriceList) {
      if (nameLower.includes('us') || nameLower.includes('united states') || nameLower.includes('america') || !usPriceList) {
    ...
    if (pl.currency === 'AUD' && !auPriceList) {
      if (nameLower.includes('au') || nameLower.includes('australia') || !auPriceList) {
  ...
  // Log all found price lists for debugging
  if (!usPriceList || !auPriceList) {
...
async function getContextualPrices(productIds, countryCode) {
  ...
  for (let i = 0; i < productIds.length; i += 50) {
    ...
    try {
      ...
      log('SHOPIFY', `Contextual pricing batch ${i} error: ${e.message}`);
    }
    await sleep(400);
  }
...
module.exports = { getAllProducts, getMarketPriceLists, getFixedPrices, getContextualPrices, setFixedPrices, shopifyGraphQL };
