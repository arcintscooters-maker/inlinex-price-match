...
function loadManualMappings() {
  ...
  for (const m of data.mappings) {
    ...
    else if (src === 'is') is.set(sku, m.shopifyMatch.toLowerCase());
  }
  log('MATCH', `Loaded ${iw.size} IW + ${xt.size} XT + ${is.size} IS manual mappings`);
  return { iw, xt, is };
}
...
function matchAll(shopifyProducts, iwProducts, xtProducts, isProducts) {
  ...
  for (const product of shopifyProducts) {
    ...
    for (const p of isProducts) {
      const manualTarget = isManualMap.get(p.sku);
      if (!manualTarget) continue;
      const matched = shopifySkus.includes(manualTarget) ||
        shopifyBarcodes.includes(manualTarget) ||
        titleLower.includes(manualTarget.toLowerCase());
      if (matched) {
        isMatch = p;
        isMethod = 'manual';
        log('MATCH', `MANUAL-IS: "${product.title}" -> "${p.name}" (Rp${p.price.toLocaleString()})`);
        break;
      }
    }
